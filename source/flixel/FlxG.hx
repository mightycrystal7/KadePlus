...
class FlxG
{
	...
	public static inline function switchState(nextState:NextState):Void
	{
		...
		{
			mod.onStateDestroy({stateName: Type.getClassName(Type.getClass(FlxG.state))});
		});
		if (!nextState.isInstance() || canSwitchTo(cast nextState))
		{
			ModuleManager.forEach(function(mod)
			{
				mod.onStateSwitchPre({stateName: Type.getClassName(Type.getClass(cast(nextState, flixel.FlxState)))});
			});
			state.startOutro(function()
			{
				ModuleManager.forEach(function(mod)
				{
					mod.onStateSwitch({stateName: Type.getClassName(Type.getClass(cast(nextState, flixel.FlxState)))});
				});
				...
				ModuleManager.forEach(function(mod)
				{
					mod.onStateSwitchPost({stateName: Type.getClassName(Type.getClass(cast(nextState, flixel.FlxState)))});
				});

				ModuleManager.forEach(function(mod)
				{
					mod.onCreate({stateName: Type.getClassName(Type.getClass(cast(nextState, flixel.FlxState)))});
				});
			});
		}
	}
