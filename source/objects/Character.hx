	...
	public function new(x:Float, y:Float, ?character:String = "bf", ?isPlayer:Bool = false)
	{
		...
		switch (curCharacter)
		{
			...
			case 'bf':
				...
				case 'bfwhite':
					var tex = Paths.getSparrowAtlas('characters/whitebf', 'shared');
					frames = tex;
					iconColor = 'FFFFFFFF';
					animation.addByPrefix('idle', 'BF idle dance', 24, false);
					animation.addByPrefix('singUP', 'BF NOTE UP0', 24, false);
			...
			case 'bfscared':
				var tex = Paths.getSparrowAtlas('characters/bfscared', 'shared');
				frames = tex;
				iconColor = 'FFD500';
				animation.addByPrefix('idle', 'BF idle dance', 24, false);
				animation.addByPrefix('singUP', 'BF NOTE UP0', 24, false);
			...
			case "impostergreen":
				frames = Paths.getSparrowAtlas('characters/green');
			...
			case "black":
				...
				case "whitegreen":
					...
					// trace(openfl.utils.Assets.exists(Paths.image("characters/green", "shared")));
					animation.addByPrefix("idle", "impostor dance left", 24, false);
	...
	override function update(elapsed:Float)
	{
		if (!isPlayer)
		{
			if (animation.curAnim != null
				&& animation.curAnim.name.startsWith('sing')) // fixes crashes if the animation doesn't exist or is null
			{
				holdTimer += elapsed;
			}
			...
			if (curCharacter == 'impostergreen')
				dadVar = 10;
			if (holdTimer >= backend.Conductor.stepCrochet * dadVar * 0.001)
			{
				dance();
				holdTimer = 0;
			}
